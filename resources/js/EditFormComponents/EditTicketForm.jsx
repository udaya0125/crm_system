...
//     // Set form values when editingTicket and users are available
//     useEffect(() => {
//         if (editingTicket) {
...
//             // Only set technician if users are loaded and we have an assigned technician
//             if (editingTicket.assigned_technician && users.length > 0) {
//                 const technicianOption = users.find(
//                     user => user.value === parseInt(editingTicket.assigned_technician)
//                 );
...
//         setTicketForm((prev) => ({ 
//             ...prev, 
//             assigned_technician: selectedOption ? selectedOption.value : null 
//         }));
//     };
...
//                             Edit Ticket
//                         </h2>
//                         {ticketForm.ticket_id && (
//                             <span className="text-sm bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full">
//                                 ID: {ticketForm.ticket_id}
//                             </span>
//                         )}
//                     </div>
//                     <button
...
//                         {/* Status Field with React Select */}
//                         <div>
//                             <label className="block text-sm font-medium text-gray-700 mb-1">
...
const EditTicketForm = ({
    ...
    const handleSubmit = async (e) => {
        ...
        // ✅ ticketForm only has clean editable fields — no image URL, no extra keys
        Object.keys(ticketForm).forEach((key) => {
    ...
    const handleStatusChange = (selectedOption) => {
        setSelectedStatus(selectedOption);
        setTicketForm((prev) => ({ ...prev, status: selectedOption ? selectedOption.value : "" }));
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="relative px-6 py-6 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white shadow-2xl">
                <div className="flex justify-between items-center mb-6 pb-4 border-b">
                    <div className="flex items-center space-x-3">
                        <h2 className="text-2xl font-bold text-stone-800">Edit Ticket</h2>
                        ...
                        {existingImageUrl && !file && (
                            <div className="mb-3 flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50/70 px-4 py-3 text-sm">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="truncate text-gray-600 text-xs font-medium leading-tight">Current attachment</p>
                                    <a
                                ...
                                <span className="text-xs text-gray-400 shrink-0">Upload below to replace</span>
                            </div>
                        )}
...
//         const formData = new FormData();
//         Object.keys(ticketForm).forEach((key) => {
//             if (key === "image") return; // skip the existing image URL string — only append real file below
//             if (ticketForm[key] !== null && ticketForm[key] !== "") {
//                 formData.append(key, ticketForm[key]);
//             }
//         });
//         if (file) formData.append("image", file); // only append if a new file was picked
...
//     const handleTechnicianChange = (selectedOption) => {
//         setSelectedTechnician(selectedOption);
//         setTicketForm((prev) => ({ ...prev, assigned_technician: selectedOption ? selectedOption.value : null }));
//     };
...
//     // Existing image from DB (URL string)
//     const existingImage = ticketForm.image || null;
