...
const EditTaskAssigned = ({ 
  ...
  const currentAssignee = editingTaskList && Array.isArray(users) 
    ? users.find(user => Number(user.id) === Number(editingTaskList.assigned_to))
    : null;
  ...
  }, [editingTaskList, user.role, showForm, setShowForm, reset, setEditingTaskList, clearErrors]);
                  ...
                  {Array.isArray(users) && users.length > 0 ? (
                    users.map((user) => (
                      <option key={user.id} value={user.id}>
                        ...
                        {editingTaskList && Number(user.id) === Number(editingTaskList.assigned_to)}
                      </option>
                    ))
                  ) : (
