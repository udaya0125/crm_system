...
const TechTicket = () => {
    ...
    // Keep a snapshot of submitted values to render the success screen
    ...
    const onSubmit = async (data) => {
        // reCAPTCHA validation (not part of react-hook-form fields)
        if (!isRecaptchaConfigured) {
            setError("recaptcha", {
                type: "manual",
                message:
            ...
            if (error.response?.data?.errors) {
                ...
                if (serverErrors.problem_description?.[0])
                    setError("description", {
                        type: "server",
                        message: serverErrors.problem_description[0],
                    });
                if (serverErrors.recaptcha_token?.[0])
                    setError("recaptcha", {
                        type: "server",
                        message: serverErrors.recaptcha_token[0],
                    });
            } else {
                alert(
                    error.response?.data?.message ||
