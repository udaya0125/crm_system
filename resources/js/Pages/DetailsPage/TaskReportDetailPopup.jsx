...
// on each task card. Measured created_at -> updated_at (the moment the
// task row was actually created, to the moment it was saved as
// Completed). NOT start_date -> updated_at — start_date is a date-only
// field with no time-of-day (stored as midnight), so pairing it with a
// real timestamp would show inflated "time since midnight" durations
// even for a task completed within minutes of being created. NOT
// start_date -> due_date either, since due_date is just the target
// deadline and would show the full scheduled window even for a task
// finished early.
const formatDuration = (totalSeconds) => {
    if (totalSeconds == null || totalSeconds < 0) return "—";
    const days = Math.floor(totalSeconds / 86400);
...
const hasDisplayableDuration = (totalSeconds) =>
    formatDuration(totalSeconds) !== "just now";
...
/**
 * One task card inside the popup. Collapsed by default.
 *
 * When the task itself is Completed AND the elapsed time is actually
 * displayable (i.e. NOT "just now"), a duration badge shows how long it
 * ACTUALLY took: created_at -> the task's own updated_at (the moment it
 * was saved as Completed). created_at is used as the start point instead
 * of start_date because start_date has no time-of-day — it would show
 * inflated "hours since midnight" numbers for tasks completed the same
...
const UserTaskCard = ({ task }) => {
    ...
    const showTaskDuration =
        isTaskCompleted && hasDisplayableDuration(taskDurationSeconds);
 ...
 * workload. Duration is created_at -> updated_at, for the same reason
 * described above UserTaskCard.
 */
