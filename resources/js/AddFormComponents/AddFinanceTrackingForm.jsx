...
const AddFinanceTrackingForm = ({
    editingTracking,
    setShowForm,   // ✅ this now calls handleCloseForm from parent (clears editingTracking too)
    setReloadTrigger,
    handleUpdate,
}) => {
    const [submitting, setSubmitting] = useState(false);
    const [trackingForm, setTrackingForm] = useState(EMPTY_FORM);

    useEffect(() => {
        if (editingTracking) {
            setTrackingForm({ ...editingTracking });
        } else {
            setTrackingForm(EMPTY_FORM);
        }
    }, [editingTracking]);

    const handleCreate = async (formData) => {
        // ✅ Fixed: was ourtracking.store
        await axios.post(route("ourfinance.store"), formData, {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setShowForm(false); // ✅ parent's handleCloseForm also resets editingTracking
        } catch (error) {
    ...
    const labelClass = "block text-xs font-semibold tracking-wider text-stone-500 uppercase mb-1";
            ...
            <div className="relative px-6 py-6 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white shadow-2xl">
                ...
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-stone-100">
                    <h2 className="text-2xl font-bold text-stone-800">
                        {editingTracking ? "Edit Tracking Item" : "Add New Tracking Item"}
                    </h2>
                    {/* ✅ Fixed: X button now closes the modal */}
                    <button
                        type="button"
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-stone-100">
                        ...
                        >
                            {submitting ? "Saving..." : editingTracking ? "Update" : "Create"}
                        </button>
                    </div>
                </form>
            </div>
