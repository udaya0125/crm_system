...
//         try {
//             await axios.post(route("domains.store"), formData, {
//                 headers: {
...
//                 await handleUpdate(formData, editingDomain.id);
//             } else {
//                 // Creating new domain
...
const AddDomainForm = ({ editingDomain, handleUpdate, setReloadTrigger, onClose }) => {
    const [submitting, setSubmitting] = useState(false);
    const [domainForm, setDomainForm] = useState(EMPTY_FORM);

    // ✅ Same pattern as AddExpirationForm
    ...
    // ✅ Transform clients for react-select — same as AddExpirationForm
    const clientOptions = clients.map((client) => ({
        value: client.id,
        label: client.organization_name || client.name,
    }));

    // ✅ Custom styles — same as AddExpirationForm
    ...
    useEffect(() => {
        if (editingDomain) {
            setDomainForm({
                ...
                register: editingDomain.register ?? "",
                purchase_date: editingDomain.purchase_date ?? "",
                expiry_date: editingDomain.expiry_date ?? "",
                auto_renewal_status: editingDomain.auto_renewal_status ?? "",
                dns_provider: editingDomain.dns_provider ?? "",
            });

            // ✅ Pre-select the client when editing — same pattern as AddExpirationForm
            if (editingDomain.client_id && clients.length > 0) {
                const found = clientOptions.find(
                    (opt) => opt.value === editingDomain.client_id
                );
    ...
    }, [editingDomain, clients]); // ✅ depends on clients so options are ready before matching
    ...
    // ✅ Handle client change — same as AddExpirationForm
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="relative px-6 py-6 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white shadow-2xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {/* ✅ Client — react-select, same as AddExpirationForm */}
                    <div>
