...
//     // data after a save). `id` is kept explicitly (separate from
//     // `clientId`) so save handlers can tell existing rows (id present) from
//     // freshly-added draft rows (id null) and send the id back to the
//     // backend — without it, the backend can't match items on update and
//     // ends up deleting + recreating every row, which resets created_at.
//     useEffect(() => {
//         setChecklistDraft(
...
//                 // Existing items carry their real DB id — send it so the
//                 // backend updates the row in place instead of recreating
//                 // it (which would reset created_at on every save).
//                 if (item.id) {
//                     formData.append(
//                         `task_items[${index}][id]`,
//                         item.id,
...
//     // Note: editing here is gated at two levels —
//     //   1. Task-level: `isLocked` (whole task is Completed) blocks ALL
//     //      checklist mutation, same as before.
//     //   2. Item-level: once a SPECIFIC item's own status is "Completed",
//     //      it is now PERMANENTLY locked — its description becomes
//     //      read-only AND it can no longer be toggled back to Pending.
//     //      There is no way to "unlock" a completed item from the UI once
//     //      it's set; this mirrors the equivalent guard added server-side
//     //      in TaskAssignedService::storeTaskItems().
//     const addDraftItem = () => {
//         if (isLocked) return;
...
//                 // Guard against editing a completed item's description —
//                 // the input is disabled in the UI, but this is a second
//                 // line of defense in case updateDraftText is ever called
//                 // programmatically.
...
//     // Once an item is marked Completed it is PERMANENTLY locked — it can
//     // no longer be toggled back to Pending. This is a one-way transition.
//     const toggleDraftStatus = (clientId) => {
//         if (isLocked) return;
//         setChecklistDraft((prev) =>
//             prev.map((i) => {
//                 if (i.clientId !== clientId) return i;
//                 if (i.status === "Completed") return i; // permanent — no-op
...
//                         the TASK is unlocked (no toggle button). Within
//                         that, each ITEM whose own status is "Completed" is
//                         PERMANENTLY locked: its description input is
//                         disabled, and its checkbox button is disabled too
//                         so it can never be toggled back to Pending. */}
//                     <div>
...
//                                                         title={
//                                                             itemLocked
//                                                                 ? "Completed — locked, cannot be reopened"
//                                                                 : "Mark as complete"
//                                                         }
//                                                     >
//                                                         {itemLocked ? (
...
//                                                         title={
//                                                             itemLocked
//                                                                 ? "Completed items are permanently locked and can't be edited"
//                                                                 : undefined
//                                                         }
...
//     // Completed tasks are locked at the TASK level: no quick-edits to
//     // status/priority/due date/title, and no "Edit ticket" access.
//     const isLocked = task.status === "Completed";
//     const canQuickEdit = typeof onQuickUpdate === "function" && !isLocked;

//     // Checklist items are a SEPARATE lock, one level down: they stay
//     // editable even after the task itself is Completed. Each item locks
//     // permanently on its own once ITS status is set to Completed (see
//     // toggleDraftStatus/updateDraftText below and the mirrored guard in
//     // TaskAssignedService::storeTaskItems() server-side). Adding/removing
//     // rows is still a structural change to the list, so that stays tied
//     // to the task-level lock (isLocked) rather than this one.
//     const canEditChecklist = typeof onQuickUpdate === "function";
...
//     // `bypassLock` lets the checklist save path go through even once the
//     // task itself is Completed — buildFormData() still re-sends every
//     // main field unchanged (task.title, task.status, etc.), so the only
//     // thing that can actually change in that payload is task_items. The
//     // server enforces this too (see TaskAssignedService::update()), so
//     // this isn't just a UI-level allowance.
//     const handleQuickSave = async (
//         overrides,
//         savingKey,
//         { bypassLock = false } = {},
//     ) => {
//         if (isLocked && !bypassLock) return false;
//         setError("");
//         setSavingField(savingKey || Object.keys(overrides)[0]);
//         try {
//             await onQuickUpdate(buildFormData(overrides), task.id);
...
//     // ---- Checklist — always-editable draft ----
//     // Note: editing here is gated at two INDEPENDENT levels now —
//     //   1. Item-level: once a SPECIFIC item's own status is "Completed",
//     //      it is PERMANENTLY locked — its description becomes read-only
//     //      AND it can no longer be toggled back to Pending. There is no
//     //      way to "unlock" a completed item from the UI once it's set;
//     //      this mirrors the equivalent guard in
//     //      TaskAssignedService::storeTaskItems().
//     //   2. Task-level (`isLocked`): once the TASK itself is Completed,
//     //      the LIST STRUCTURE is frozen — no adding or removing rows —
//     //      but existing, not-yet-completed rows can still be edited and
//     //      marked complete. This is the behavior change from before,
//     //      where isLocked used to block the checklist entirely too.
//     const addDraftItem = () => {
//         if (isLocked) return;
//         setChecklistDraft((prev) => [
//             ...prev,
...
//                 // the input is disabled in the UI, but this is a second
//                 // line of defense in case updateDraftText is ever called
//                 // programmatically. Note: this is the ONLY guard here now
//                 // — task-level `isLocked` no longer blocks this function,
//                 // so descriptions stay editable after the task itself is
//                 // Completed, as long as the item isn't Completed too.
//                 if (i.status === "Completed") return i;
//                 return { ...i, description: text };
//             }),
//         );
...
//     // Once an item is marked Completed it is PERMANENTLY locked — it can
//     // no longer be toggled back to Pending. This is a one-way transition,
//     // and it's the only thing that stops an item from being editable —
//     // task-level `isLocked` no longer applies here.
//     const toggleDraftStatus = (clientId) => {
//         setChecklistDraft((prev) =>
//             prev.map((i) => {
//                 if (i.clientId !== clientId) return i;
...
//     // Allowed even when the task is Completed — bypassLock: true. The
//     // only thing this can actually change server-side, once locked, is
//     // task_items (main fields are re-sent unchanged by buildFormData, and
//     // the server independently re-freezes them anyway — see
//     // TaskAssignedService::update()).
//     const saveChecklist = async () => {
//         const items = checklistDraft
//             .filter((i) => i.description.trim() !== "")
//             .map((i) => ({
...
//                     {/* Checklist — editable draft whenever an onQuickUpdate
//                         handler exists (canEditChecklist), REGARDLESS of
//                         whether the task itself is Completed. Only two
//                         things change once the task is locked:
//                           - "Add item" and per-row delete are hidden (list
//                             structure is frozen)
//                           - each row's own itemLocked state is unaffected —
//                             it was always independent of the task lock. */}
//                     <div>
//                         <div className="flex justify-between items-center mb-2">
//                             <div className="flex items-center gap-2">
//                                 <p className="text-[11px] font-mono uppercase tracking-wider text-gray-400">
...
const TaskDetailPopup = ({
    ...
    // Seed the review form from the task's previous verdict whenever the
    // popup switches to a different task.
    useEffect(() => {
        resetReviewForm({
            admin_status: task?.admin_status
                ? ADMIN_STATUS_OPTIONS.find(
    ...
    // Don't show a countdown/overdue badge once the task is done — it's
    // meaningless after completion and was showing things like "3d
    // overdue" on tasks that were already finished.
    const dueInfo = isLocked ? null : getDueInfo(task.due_date);
