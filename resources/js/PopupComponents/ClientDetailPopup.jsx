...
const SectionLabel = ({ label }) => (
    <p className="text-xs font-semibold uppercase tracking-widest mb-1 mt-4"
        style={{ color: "#0d77c3" }}>
        {label}
    </p>
);

const ClientDetailPopup = ({ client, onClose }) => {
    if (!client) return null;

    const hasValue = (val) =>
        val !== null && val !== undefined && val !== "" && val !== "N/A";

    const fullAddress = [client.street, client.city, client.state, client.country]
        .filter(hasValue)
        .join(", ");

    const phones = [
        client.mobile && { label: "Mobile", value: client.mobile, icon: Smartphone },
                ...
                <div className="flex-1 overflow-y-auto -mt-4 bg-white rounded-t-2xl px-6 pt-4 pb-6">
                    ...
                    {(hasValue(client.code) || hasValue(client.pannumber)) && (
                        <>
                            ...
                            <FieldRow>
                                <Field icon={Hash} label="Code" value={client.code} />
                                <Field icon={CreditCard} label="PAN Number" value={client.pannumber} />
                            </FieldRow>
                        </>
                    )}
                    ...
                    {(hasValue(client.name) || hasValue(client.type)) && (
                        <>
                            ...
                            <FieldRow>
                                <Field icon={User} label="Name" value={client.name} />
                                <Field icon={Tag} label="Type" value={client.type} />
                            </FieldRow>
                            <FieldRow>
                                <Field icon={GitBranch} label="Branch" value={client.branchname} />
                                <Field icon={BookOpen} label="Ledger Name" value={client.ledgername} />
                            </FieldRow>
                        </>
                    )}

                    {/* Contact section */}
                    {(hasValue(client.email) || phones.length > 0 || hasValue(client.website)) && (
                        <>
                            ...
                            {phones.length > 0 && (
                                <FieldRow>
                                    {phones.slice(0, 2).map((p) => (
                                        <Field key={p.label} icon={p.icon} label={p.label} value={p.value} />
                                    ))}
                                </FieldRow>
                            )}
                            {phones.length === 3 && (
                                <FieldRow>
                                    <Field icon={phones[2].icon} label={phones[2].label} value={phones[2].value} />
                                </FieldRow>
                            )}
                        </>
                    )}
